...
class LinearBottleneck(nn.Module):
    def __init__(self, inplanes, outplanes, stride=1, t=6):
        ...
        self.conv1 = tdLayer(nn.Conv2d(inplanes, inplanes * t, kernel_size=1, bias=False))
        self.bn1 = tdBatchNorm(inplanes * t)
        self.conv2 = tdLayer(nn.Conv2d(inplanes * t, inplanes * t, kernel_size=3, stride=stride, padding=1, bias=False,
                               groups=inplanes * t))
        self.bn2 = tdBatchNorm(inplanes * t)
        self.conv3 = tdLayer(nn.Conv2d(inplanes * t, outplanes, kernel_size=1, bias=False))
        self.bn3 = tdBatchNorm(outplanes)
        self.activation = LIFSpike()
        self.stride = stride
        self.t = t
...
class test(nn.Module):
    ...
    def __init__(self, scale=1.0, input_size=224, t=6, in_channels=3, num_classes=10):
        """
        ...
        self.conv1 = tdLayer(nn.Conv2d(in_channels, self.c[0], kernel_size=3, bias=False, stride=self.s[0], padding=1),tdBatchNorm)
        self.bn1 = tdBatchNorm(self.c[0])
        ...
        self.conv_last = tdLayer(nn.Conv2d(self.c[-1], self.last_conv_out_ch, kernel_size=1, bias=False))
        self.bn_last = tdBatchNorm(self.last_conv_out_ch)
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.dropout = nn.Dropout(p=0.2, inplace=True)  # confirmed by paper authors
        self.fc = SeqToANNContainer(nn.Linear(self.last_conv_out_ch, self.num_classes))
        self.init_params()
